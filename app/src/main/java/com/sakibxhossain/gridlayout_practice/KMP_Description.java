...
public class KMP_Description extends AppCompatActivity {

    MeowBottomNavigation kmp_bottom_navigation;
    ImageView back_btn;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_kmp_description);

        kmp_bottom_navigation = findViewById(R.id.kmp_bottom_bar);
        back_btn = findViewById(R.id.kmp_back_btn);

        kmp_bottom_navigation.add(new MeowBottomNavigation.Model(0, R.drawable.algorithm_bottom_icon));
        kmp_bottom_navigation.add(new MeowBottomNavigation.Model(1, R.drawable.code_bottom_nav_icon));
        kmp_bottom_navigation.add(new MeowBottomNavigation.Model(2, R.drawable.problems_bottom_bar_icon));
        kmp_bottom_navigation.show(0,true);

        kmp_bottom_navigation.setOnShowListener(new Function1<MeowBottomNavigation.Model, Unit>() {
            @Override
            public Unit invoke(MeowBottomNavigation.Model model) {
                Fragment fragment;

                if(model.getId() == 0){
                    fragment = new Algorithm_Fragment("kmp");
                    loadAlgorithm(fragment);
                }else if(model.getId() == 1){
                    fragment = new code_fragment("kmp");
                    loadCode(fragment);

                }else if(model.getId() == 2){
                    fragment = new Problems_Fragment("kmp");
        ...
        back_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent goBack_to_Sorting = new Intent(KMP_Description.this, Strings_Algorithm.class);
...
//        fragment.setArguments(data2);
        fragmentTransaction.replace(R.id.kmp_frameLayout,fragment, null)
                .commit();
    }
